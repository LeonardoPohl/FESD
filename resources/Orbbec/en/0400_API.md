...
3. Norm 3 , in case VideoStream is not destroyed, just execute the start and stop functions of VideoStream , the optimal choice option:
...
#### Get the Harley camera to save calibration parameters

| interface class | API                                                          | Parameter | Description                                                  |
| --------------- | ------------------------------------------------------------ | --------- | ------------------------------------------------------------ |
| Device          | getOBCameraParams(CameraParamsDepthType depthRes, CameraParamsColorType  colorRes) | Yes       | This interface is currently only available for the Harley modules (Orbbec Dabai DW/ Orbbec Dabai DCW) and cannot be used to read camera calibration parameters using the "16.1.14" interface.  As there are multiple resolutions of camera parameters in the Harley module, the correct camera parameters are returned by matching the different parameter resolutions of the depth CameraParamsDepthType and Rgb CameraParamsColorType.<br/><br/>Translated with www.DeepL.com/Translator (free version) |

**Description of interface parameters:**

  DCW module CameraParamsDepthType and CameraParamsColorType matching parameters：

| **Serial No.** | CameraParamsDepthType                                        | CameraParamsColorType                | **Remarks**                                                  |
| -------------- | ------------------------------------------------------------ | ------------------------------------ | ------------------------------------------------------------ |
| **1**          | CAMERA_PARAMS_DEPTH_RES_1024_768  = 0,  CAMERA_PARAMS_DEPTH_RES_512_384  = 1,  CAMERA_PARAMS_DEPTH_RES_640_480  = 2,  CAMERA_PARAMS_DEPTH_RES_320_240  = 3,  CAMERA_PARAMS_DEPTH_RES_160_120  = 4,  CAMERA_PARAMS_DEPTH_RES_480_360  = 5,  CAMERA_PARAMS_DEPTH_RES_240_180  = 6 | CAMERA_PARAMS_COLOR_RES_DEFAULT  = 0 | If a non-matching pass is selected first, the Read Camera Parameters interface returns an exception.  The depth internal reference of this parameter is used as the depth to point cloud internal reference |
| **2**          | CAMERA_PARAMS_DEPTH_RES_640_360  = 7                         | CAMERA_PARAMS_COLOR_RES_640_360  = 1 | The Read Camera Parameters interface returns an exception if a non-matching pass-through parameter is selected first.  The color internal reference to this parameter is used as the internal reference to the RGBD data |
| **3**          | CAMERA_PARAMS_DEPTH_RES_320_180  = 8                         | CAMERA_PARAMS_COLOR_RES_320_180  = 2 |                                                              |

 

DW module CameraParamsDepthType and CameraParamsColorType matching instructions for passing parameters：

| Number | CameraParamsDepthType                                        | CameraParamsColorType                | **Remarks**                                                  |
| ------ | ------------------------------------------------------------ | ------------------------------------ | ------------------------------------------------------------ |
| **1**  | CAMERA_PARAMS_DEPTH_RES_1024_768  = 0,  CAMERA_PARAMS_DEPTH_RES_512_384  = 1,  CAMERA_PARAMS_DEPTH_RES_640_480  = 2,  CAMERA_PARAMS_DEPTH_RES_320_240  = 3,  CAMERA_PARAMS_DEPTH_RES_160_120  = 4,  CAMERA_PARAMS_DEPTH_RES_480_360  = 5,  CAMERA_PARAMS_DEPTH_RES_240_180  = 6 | CAMERA_PARAMS_COLOR_RES_DEFAULT  = 0 | If a non-matching pass is selected first, the Read Camera Parameters interface returns an exception. |

Code example (reading depth resolution 240x180 point cloud internal reference):

OBCameraParams cameraParams = Device.getOBCameraParams(CAMERA_PARAMS_DEPTH_RES_240_180, CAMERA_PARAMS_COLOR_RES_DEFAULT);
