...
# eval
def val(eval_loader, model, criterion, epoch, epochs, writer):
    ...
    for i, pack in enumerate(eval_loader, start=1):        
        ...
        writer.add_scalar('Loss/val', loss_record.show(), i)
        writer.add_scalar('MSE/val', sum(sum((pred_s - gt)**2) / len(pred_s)) / rgbs.shape[0], i)
