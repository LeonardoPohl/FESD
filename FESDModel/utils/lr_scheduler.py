...
def get_scheduler(optimizer, n_iter_per_epoch, lr_scheduler, lr_decay_epochs, lr_decay_steps, lr_decay_rate, epochs, warmup_epoch, warmup_multiplier):
    if "cosine" in lr_scheduler:
        ...
        if len(lr_decay_epochs) > 0:
            lr_decay_epochs = lr_decay_epochs
        else:
            lr_decay_epochs = [lr_decay_steps * i for i in range(1, epochs // lr_decay_steps)]
        scheduler = MultiStepLR(
            optimizer=optimizer,
            gamma=lr_decay_rate,
            milestones=[(m - warmup_epoch) * n_iter_per_epoch for m in lr_decay_epochs])
    else:
